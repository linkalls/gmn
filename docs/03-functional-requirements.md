...
- **対応モデル**:
  ...
  - `gemini-2.5-pro` - 高性能
  - `gemini-3-pro-preview` - 最新、コーディング向け（Standard tier）
- **優先度**: 必須
...
### FR-006: Gemini 3 Pro 対応

- **概要**: Gemini 3 Pro モデルの `thoughtSignature` を正しく処理する
- **処理**:
  - API レスポンスから `thoughtSignature` を抽出
  - ツール呼び出し時に署名を保持して履歴に追加
  - 後続リクエストで署名を含めて送信
- **優先度**: 必須
...
### FR-601: list_directory ツール

- **概要**: ディレクトリの内容を一覧表示する
- **パラメータ**: `path` - 対象ディレクトリパス
- **出力**: ファイル/ディレクトリ名、サイズ、種別
- **確認**: 不要
- **優先度**: 高

### FR-602: read_file ツール
...
### FR-603: write_file ツール

- **概要**: ファイルに内容を書き込む
- **パラメータ**: `path`, `content`
- **処理**: 既存ファイルは上書き、新規作成も可
- **確認**: **必要**（ユーザー承認）
- **優先度**: 高

### FR-604: edit_file ツール

- **概要**: ファイルの特定テキストを置換する
- **パラメータ**: `path`, `old_text`, `new_text`
- **処理**: `old_text` を `new_text` に置換
- **確認**: **必要**（ユーザー承認）
- **優先度**: 高

### FR-605: glob ツール

- **概要**: パターンにマッチするファイルを検索する
- **パラメータ**: `pattern` - glob パターン（例: `**/*.go`）
- **出力**: マッチしたファイルパス一覧
- **確認**: 不要
- **優先度**: 中

### FR-606: search_file_content ツール

- **概要**: ファイル内でテキスト/正規表現を検索する
- **パラメータ**: `pattern`, `path`, `regex`（オプション）
- **出力**: マッチした行とコンテキスト
- **確認**: 不要
- **優先度**: 中
...
| 高     | FR-601  | list_directory ツール          |
| 高     | FR-602  | read_file ツール               |
| 高     | FR-603  | write_file ツール              |
| 高     | FR-604  | edit_file ツール               |
| 高     | FR-305  | ツール発見                     |
| 高     | FR-306  | ツール実行                     |
| 中     | FR-203  | ディレクトリ読み込み           |
