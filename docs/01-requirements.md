...
Go 言語で開発された高速・軽量な Gemini CLI ツールを提供する。
非対話型の高速実行と、対話型のリッチなTUIの両方をサポート。

### 1.3 背景

- Go言語のネイティブバイナリにより高速起動とメモリ効率を実現
- 非対話型（スクリプト/自動化）用途での高速起動を重視
- 対話型用途でのモダンなTUI体験を提供
- ワンバイナリで配布可能な軽量ツール
...
- 前提条件: Google OAuth認証済み（~/.gemini/ に認証情報が存在）
- 基本フロー:
...
- Google OAuth 認証が完了済み
- `~/.gemini/` ディレクトリに認証情報が存在（公式 Gemini CLI等で設定可能）
